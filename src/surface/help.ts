...
function buildQuickstart(config: ResolvedConfig, locale: HelpLocale): string[] {
  ...
  return locale === "zh"
    ? [
        ...
        "做论文检索时先用 academic；Paperflow 项目把选中的结果交给其目录/书目适配器，不要让搜索核心直接写项目路径。",
        "真实检索默认记历史；只有明确不需要时才用 --no-history 或 recordHistory=false。run academic_search 是显式始终持久化的形式，引用雪球用 citation plan/run/resume。",
    ...
    : [
        ...
        "Use academic first for paper search. In Paperflow projects, send selected results through a project-side bibliography/catalog adapter that resolves path roles rather than writing project paths from search core.",
        "Real discovery is recorded by default; use --no-history or recordHistory=false only for an explicit opt-out. run academic_search is the explicitly always-durable form; use citation plan/run/resume for snowballing.",
...
export async function createHelpSnapshot(
  ...
  const notes = locale === "zh"
    ? [
        ...
        "resource_add / collection_list 是兼容本地 sink；Paperflow 负责项目路径角色，已选记录由项目侧书目/目录工具管理。",
    ...
    : [
        ...
        "resource_add / collection_list are compatibility local sinks; Paperflow owns project path roles while a project-side bibliography/catalog tool owns selected records.",
